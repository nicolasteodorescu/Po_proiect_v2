...
Team::Team(const std::string& name, std::vector<std::shared_ptr<Champion>> r)
    : teamName(name), roster(std::move(r)) {
    numberTeams++;
}
...
void Team::removeChampion(const std::string& name) {
    auto it = std::find_if(roster.begin(), roster.end(),[&name](const auto& c) {
...
double Team::getAverageWinRate() const {
    if (roster.empty())
        return 0.0;
    double sum = std::accumulate(roster.begin(), roster.end(), 0.0,[](double acc, const auto& c) {
...
int  Team::getSize() const {
    return static_cast<int>(roster.size());
}
...
DraftTeam::DraftTeam(const DraftTeam& other)
    : Team(other),
...
int  DraftTeam::getCurrentPickIndex() const {
